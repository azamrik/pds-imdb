...
basics_ratings <-
  filter(as_tibble(subset(
    read_csv("basics_ratings.csv", na = "\\N"),
    select = -c(
      endYear,
      runtimeMinutes,
      isAdult,
...
aggRatings <-
  aggregate(titleId ~ genre + averageRatingInt, basics_ratings, function(x)
    length(unique(x)))
names(aggRatings)[names(aggRatings) == "titleId"] <- "Titles"
names(aggRatings)[names(aggRatings) == "averageRatingInt"] <-
  "AvgRating"
names(aggRatings)[names(aggRatings) == "genre"] <- "Genre"
genreList <-
  sort(unique(basics_ratings$genre, incomparables = FALSE))

###############################
### scatter plot  variables ###
###############################
principals <-
  subset((read_csv("principals.csv", na = "\\N")), select = -c(characters))
actorNames <-
  subset((read_csv("names.csv", na = "\\N")),
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      tags$p("This application is designed by The Reelers for the purpose of visualizing the film industry using data provided by IMDB. In this Shiny application users can:",
      tags$ol(
        tags$li("View the distribution of movies with different ratings based on genres."),
        tags$li("View the average ratings trend over the years."),
        tags$li("View the cast's average ratings for different movies."),
        tags$li("View the connections between directors and cast members for different movies.")
      ),
      tags$p("Developed by Developers: Ahmad Zamrik, Wong Wai Cheng, Wai Chee San, Nik Syen Chyn"),
      tags$p(
        "Data source:",
        tags$a(href="https://www.imdb.com/interfaces/", "imdb.com/interfaces/")
      )
      )
    ),
    ...
    mainPanel(
      width = 9,
      fluidRow(column(
        7, plotlyOutput("interactiveGenre", height = "250px")
      ),
      column(
        5, plotlyOutput("scaterPlot", height = "250px")
      )),
      fluidRow(
...
server <- function(input, output, session) {
  ...
  selectedRow <- reactive({
    if (is.null(input$moviesTable_rows_selected)) {
      return((((
        c(
          dplyr::filter(basics_ratings, genre %in% input$groupCheckBox) %>%
    ...
    ))[[1]])[c(input$moviesTable_rows_selected)]
  })
  ...
  output$scaterPlot <- renderPlotly({
    ...
    movie <- inner_join(
      basics_ratings,
      principals %>% filter(nconst %in% castList),
      by = c("titleId" = "tconst")
    )
    movie <-
      inner_join(actorNames, movie, by = c("nameId" = "nconst"))
    plotDf <- movie %>%
      ...
      ggplot(
        ...
        aes(
          ...
          text = paste(
            ...
            genre,
            "\nYear:\t",
            as.numeric(as.character(startYear)),
            "\nAvg Rating:\t",
            averatings
          )
        )
  ...
  output$range <- renderPrint(((selectedRow())))
  output$network <- renderForceNetwork({
    ...
    nodes <-
      sqldf(
        "select nameId as nconst, primaryName from actorNames
    ...
    nodes3 <-
      unique(nodes_new %>% left_join(directors, by = c('value' = 'nconst')))
